...
Model using a non-trainable time-dependent encoder
...
from src.encode_decode.nontrainable_enc_sigma4_param import NonTrainableEncSigma4Param
from src.nn_models.score_unet import ScoreUNet, ScoreUNetAlt


class Sigma4VNT(nn.Module):
    ...
    def setup(self):
        ...
        self.encdec = NonTrainableEncSigma4Param(self.encdec_config,
                                             config.gamma_min,
                                             config.gamma_max,
                                             self.input_shape[0])
    ...
    def __call__(self, 
        ...
        x_hat = jnp.sqrt(alpha_squared)*z_t - sigma_t * v_hat
        
        norm_loss = jnp.sum(jnp.square(v_t - v_hat + sigma_t * (x_hat - alpha_squared * init_x)), axis = (1, 2, 3))        
        
        loss_diff = 0.5 * g_t_grad * jnp.squeeze(alpha_squared) * norm_loss
                
        # Not used in this version
        g_t_grad_min_mean_max = (0, 0, 0)
    ...
    def initial_encode(self, x: NDArray):
        return self.encdec.initial_encode(x)
