...
# Using v parametrization for loss and sampling
class VDM2V(nn.Module):
    ...
    def __call__(self, 
        ...
        v_t = jnp.sqrt(1. - var_t) * eps - jnp.sqrt(var_t) * init_x
        ...
        loss_diff = 0.5 * g_t_grad * jnp.squeeze((1. - var_t)) * loss_diff_mse
    ...
    def sample(self, i: int, T: int, 
        ...
        #   + \alpha_s(1 - exp(g_s - g_t)) * x_hat
        # mu_P = \alpha_s/\alpha_t * (exp(g_s - g_t) - 1 + 1) * z_t 
        #   - \alpha_s(exp(g_s - g_t) - 1) * x_hat
        # where
        ...
        mu_P = alpha_s/alpha_t * (snr_t_s_m1 + 1.) * z_t - alpha_s * snr_t_s_m1 * x_hat 
        ...
        if dynamic_thresholding:
            ...
            if p_val > 1:
                z_s = jnp.clip(z_s, a_min = -p_val, a_max = p_val)/p_val
